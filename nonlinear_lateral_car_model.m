...
%     dx(4,1) = sin(psi)*x_dot+cos(psi)*y_dot;
    ...
    % The nonlinear equation describing the dynamics of the drone
    dx(1,1) = y_dot;
    dx(2,1) = -(2*Caf+2*Car)/(m*x_dot)*y_dot+(-x_dot-(2*Caf*lf-2*Car*lr)/(m*x_dot))*psi_dot+2*Caf/m*U1;
    dx(3,1) = psi_dot;
    dx(4,1) = -(2*lf*Caf-2*lr*Car)/(Iz*x_dot)*y_dot-(2*lf^2*Caf+2*lr^2*Car)/(Iz*x_dot)*psi_dot+2*lf*Caf/Iz*U1;
